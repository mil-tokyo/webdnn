...
kernel void sgemm_large_k(const device int *MNK[[buffer(0)]],
                          const device float *ab[[buffer(1)]],
                          const device float *A[[buffer(2)]],
                          const device float *B[[buffer(3)]],
                          device float *C[[buffer(4)]],
                          ...
                          uint2 group_position[[threadgroup_position_in_grid]])
{
    ...
    const int N = MNK[1];
    const int K = MNK[2];
    const int m = group_position.x;
    const int n = group_position.y;

    threadgroup float shared[64];
    float sum = 0;

    for (int k = index; k < K; k += 64)
    {
        sum += A[k * M + m] * B[n * K + k];
    }
    shared[index] = sum;
    ...
    threadgroup_barrier(mem_flags::mem_none);
    if (index < 32)
        shared[index] += shared[index + 32];
    threadgroup_barrier(mem_flags::mem_none);
    if (index < 16)
        shared[index] += shared[index + 16];
    threadgroup_barrier(mem_flags::mem_none);
    if (index < 8)
        shared[index] += shared[index + 8];
    threadgroup_barrier(mem_flags::mem_none);
    if (index < 4)
        shared[index] += shared[index + 4];
    threadgroup_barrier(mem_flags::mem_none);
    if (index < 2)
        shared[index] += shared[index + 2];
    threadgroup_barrier(mem_flags::mem_none);
    if (index == 0)
        C[n * M + m] = ab[0] * (shared[0] + shared[1]) + ab[1] * C[n * M + m];
}
