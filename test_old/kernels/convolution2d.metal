    ...
    for (int c1 = 0; c1 < C1; c1++)
    {
        for (int kh = 0; kh < KH; kh++)
        {
            ...
            for (int kw = 0; kw < KW; kw++)
            {
                ...
                result += W[((c2 * C1 + c1) * KH + kh) * KW + kw] * X[((c1 * N + n) * H1 + h1) * W1 + w1];
            }
        }
    }

    Y[((c2 * N + n) * H2 + h2) * W2 + w2] = result;
}
       ...
       uint gid[[thread_position_in_grid]])
{
    ...
    if (gid >= C1 * KH * KW * N * H2 * W2)
        return;

    const int w2 = gid % W2;
    const int h2 = gid / W2 % H2;
    const int n = gid / W2 / H2 % N;
    const int kw = gid / W2 / H2 / N % KW;
    const int kh = gid / W2 / H2 / N / KW % KH;
    const int c1 = gid / W2 / H2 / N / KW / KH;

    const int h1 = h2 * SH - PH + kh;
    const int w1 = w2 * SW - PW + kw;

    col[gid] = (h1 < 0 || h1 >= H1 || w1 < 0 || w1 >= W1)
                   ? 0
                   : im[((c1 * N + n) * H1 + h1) * W1 + w1];
}