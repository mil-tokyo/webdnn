                    ...
                    uint gid[[thread_position_in_grid]])
{
    ...
    for (int c1 = 0; c1 < C1; c1++)
    {
        ...
        {
            const int h1 = h2 * SH - kh;
            if (h1 < 0 || h1 >= H1)
                continue;

            for (int kw = 0; kw < KW; kw++)
            {
                const int w1 = w2 * SW - kw;
                if (w1 < 0 || w1 >= W1)
                    continue;
...
kernel void
convolution2d_naive_and_relu(const device Convolution2DDescriptor *desc[[buffer(0)]],
                             const device float *X[[buffer(1)]],
                             const device float *W[[buffer(2)]],
                             device float *Y[[buffer(3)]],
                             uint gid[[thread_position_in_grid]])
{
    ...
    Y[((n * C2 + c2) * H2 + h2) * W2 + w2] = result > 0 ? result : 0;
}
