...
def test_single_scale():
    scale = ConstantScale("bias", {"value": 2})
    ...
    flag_changed = True
    while flag_changed:
        graph, flag_changed = UpgradeConstantScaleToAffineTransform().optimize(graph)
    ...
    assert ops[0].parameters["scale"] == scale.parameters["value"]
    assert ops[0].parameters["bias"] == 0
