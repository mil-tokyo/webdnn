...
def template(description: str = ""):
    ...
    vy = chainer.functions.arcsin(vx)
    ...
    generate_kernel_test_case(
        description=f"[chainer] F.arcsin {description}",
        graph=graph,
        inputs={x: vx.data},
        expected={y: vy.data},
        EPS=1e-2
...
def test_with_placeholder():
    vx = chainer.Variable(np.random.rand(10, 11, 12).astype(np.float32) * 2 - 1)
    vy = chainer.functions.arcsin(vx)
    ...
    py = chainer.functions.arcsin(px)
    ...
    C.value = 12
    generate_kernel_test_case(
        description=f"[chainer] F.arcsin with placeholder",
        graph=graph,
