...
def template(axis, keepdims, description: str = ""):
    ...
    vy = chainer.functions.min(vx, axis=axis, keepdims=keepdims)
    ...
    generate_kernel_test_case(
        description=f"[chainer] F.min {description}",
        graph=graph,
...
def test_with_placeholder():
    ...
    vy = chainer.functions.min(vx, axis=1, keepdims=False)

    A = Placeholder(label="A")
    B = Placeholder(label="B")
    C = Placeholder(label="C")
    px = PlaceholderVariable([A, B, C])
    py = chainer.functions.min(px, axis=1, keepdims=False)
    ...
    C.value = 12
    generate_kernel_test_case(
        description=f"[chainer] F.min with placeholder",
        graph=graph,
