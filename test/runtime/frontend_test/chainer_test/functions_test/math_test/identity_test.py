...
def template(description: str = ""):
    ...
    vy = chainer.functions.identity(vx)
    ...
    generate_kernel_test_case(
        description=f"[chainer] F.identity {description}",
        graph=graph,
        inputs={x: vx.data},
        expected={y: vy.data},
    )
...
def test_with_placeholder():
    ...
    py = chainer.functions.identity(px)
    ...
    C.value = 12
    generate_kernel_test_case(
        description=f"[chainer] F.identity with placeholder",
        graph=graph,
