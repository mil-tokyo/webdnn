...
def test():
    ...
    vy = chainer.functions.average_pooling_2d(vx, ksize=2, stride=2)
...
def test_padding_not_zero():
    vx = chainer.Variable(np.random.rand(2, 4, 6, 8))
    vy = chainer.functions.average_pooling_2d(vx, ksize=2, stride=2, pad=1)
...
def test_stride_is_none():
    vx = chainer.Variable(np.random.rand(2, 4, 6, 8))
    vy = chainer.functions.average_pooling_2d(vx, ksize=2, stride=None, pad=1)
...
def test_irregular_size():
    vx = chainer.Variable(np.random.rand(2, 4, 6, 8))
    vy = chainer.functions.average_pooling_2d(vx, ksize=(3, 4), stride=(1, 2), pad=(1, 3))
    ...
    generate_kernel_test_case(
        description=f"[chainer] F.average_pooling_2d(ksize=(3,4), stride=(1,2), padding=(1,3))",
        graph=graph,
        inputs={x: ConstantVariable(vx.data, OrderNCHW).change_order(x.order).data},
        expected={y: ConstantVariable(vy.data, OrderNCHW).change_order(y.order).data}
    )
