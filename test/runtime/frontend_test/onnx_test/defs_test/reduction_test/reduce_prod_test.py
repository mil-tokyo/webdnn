...
def template(x_shape, axes, keepdims=None, description: str = ""):
    ...
    vy = np.prod(vx, axis=tuple(axes), keepdims=True if keepdims is None else keepdims)
    ...
    operator = make_node("ReduceProd", ["x"], ["y"], **kwargs)
    ...
    generate_kernel_test_case(
        description=f"[ONNX] ReduceProd {description}",
        graph=graph,
