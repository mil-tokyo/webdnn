...
def template(x_shape, axes, keepdims=None, description: str = ""):
    ...
    vy = np.min(vx, axis=tuple(axes), keepdims=True if keepdims is None else keepdims)
    ...
    operator = make_node("ReduceMin", ["x"], ["y"], **kwargs)
    ...
    generate_kernel_test_case(
        description=f"[ONNX] ReduceMin {description}",
        graph=graph,
