...
def template(x_shape, axes, keepdims=None, description: str = ""):
    ...
    vy = np.sum(vx, axis=tuple(axes), keepdims=True if keepdims is None else keepdims)
    ...
    operator = make_node("ReduceSum", ["x"], ["y"], **kwargs)
    ...
    generate_kernel_test_case(
        description=f"[ONNX] ReduceSum {description}",
        graph=graph,
