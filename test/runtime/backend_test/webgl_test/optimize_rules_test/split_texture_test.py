...
@with_setup(_set_texture_size(1024), _set_texture_size(config.WEBGL_MAX_TEXTURE_SIZE))
def test_tensorwise():
    x1 = chainer.Variable(np.random.rand(32*16, 32, 32, 32).astype(np.float32))
    x2 = chainer.Variable(np.random.rand(32*16, 32, 32, 32).astype(np.float32))
    h = chainer.functions.concat([x1, x2], axis=1)
    h = h + 1
    y = chainer.links.Linear(64*32*32, 1)(h)
