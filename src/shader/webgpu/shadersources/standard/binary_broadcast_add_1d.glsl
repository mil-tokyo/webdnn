...
void main() {
  uint len = meta.len;
  uint outShape0 = meta.outShape0;
  uint inAStride0 = meta.inAStride0;
  uint inBStride0 = meta.inBStride0;
  for (uint i = gl_GlobalInvocationID.x; i < len; i += 4096) {
    uint dim0 = i;
    array_c.numbers[i] = array_a.numbers[dim0 * inAStride0] + array_b.numbers[dim0 * inBStride0];
  }
}
