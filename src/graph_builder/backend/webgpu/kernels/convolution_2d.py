                          ...
                          uint num_threads[[threads_per_grid]])
{
    ...
    const int K = %%META_LOAD(convolution_2d_K)%%;
    const int S = %%META_LOAD(convolution_2d_S)%%;
    const int P = %%META_LOAD(convolution_2d_P)%%;
        ...
        for (int kh = 0; kh < K; kh++) {
            ...
            for (int kw = 0; kw < K; kw++) {
                ...
                for (int c1 = 0; c1 < C1; c1++) {
                    sum += X[((n * H1 + h1) * W1 + w1) * C1 + c1] * W[((kh * K + kw) * C2 + c2) * C1 + c1];
                }
            }
        }
...
def convolution_2d(op: Linear,
    ...
    assert w.variable.axis_order == VA.OrderHWNC, \
        f"[WebGPU] Convolution2D operator supports only OrderHWNC for data order of filter variable. " + \
    ...
    metabuffer_injector.register({
        ...
        "convolution_2d_K": op.parameters["ksize"][0],
        "convolution_2d_S": op.parameters["stride"][0],
        "convolution_2d_P": op.parameters["padding"][0],
    })
