...
def _generate_source(xs: List[Allocation], y: Allocation, codes: List[List[Any]], parameters: Dict[str, Union[int, float]]):
    ...
    variable_declaration_expression = "\n".join(
        [f"const device float *X{i} = %%LOAD_BUFFER(elementwise_Xs, {i})%%;" for i in range(len(xs))])
    ...
    for code in codes:
        if code[0] == "declare":
            ...
            if isinstance(value, int):
                ...
                elementwise_body.append(f"const float {varname} = *((device float *)(&%%LOAD_BUFFER({code[2]})%%)); // {code}")
...
@WebGPUDescriptorGenerator.register_handler(MergedElementwise)
