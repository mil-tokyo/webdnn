...
import { DataArrayConstructor, DataArrayTypes } from "../../../../interface/core/constants";
...
abstract class ReduceOp extends OperatorImpl {
  ...
  protected async runCore(context: WebDNNCPUContext, input: CPUTensor, sortedAxes: number[]): Promise<Tensor[]> {
    ...
    for (let i = 0; i < sortedAxes.length; i++) {
      ...
      const newOutputData = new DataArrayConstructor[input.dataType](outerLength*innerLength);
...
class ReduceOp13 extends ReduceOp {
  ...
  initialize(attribute: onnx.IAttributeProto[]): void {
    super.initialize(attribute);
    this.keepdims = getAttrInt(attribute, "keepdims", 1) !== 0;
    this.noopWithEmptyAxes = getAttrInt(attribute, "noop_with_empty_axes", 0) !== 0;
  }
