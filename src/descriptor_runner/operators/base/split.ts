...
abstract class Split extends OperatorImpl {
  ...
  protected calcShapeBase(input: Tensor, nOutputs: number, splitSrc: ReadonlyArray<number>) {
    let { axis } = this;
    if (axis < 0) {
      axis += input.ndim;
    }
...
export abstract class Split13 extends Split {
  ...
  protected calcShape(input: Tensor, nOutputs: number, splitTensor?: CPUTensor) {
