...
def _setup_model(model_type):
    ...
    if model_type == "conv":
        ...
        model = Sequential()
        model.add(AtrousConv2D(8, kernel_size=(3, 3), atrous_rate=(2, 2), activation="relu",
...
def generate_graph(model_type, output_dir):
    ...
    graph = KerasConverter(batch_size=None).convert(model)  # None to indicate batch_size will be placeholder label "N"
    return model, graph
...
def main():
    ...
    print("Currently, webgl backend does not support placeholder")
