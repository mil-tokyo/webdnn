...
var BaseBenchmark = function() {
    function BaseBenchmark(name, numIteration) {
...
var KerasJSBenchmark = function(_BaseBenchmark) {
    ...
    function KerasJSBenchmark(name, numIteration, flagGPU) {
        _classCallCheck(this, KerasJSBenchmark);

        var _this3 = _possibleConstructorReturn(this, (KerasJSBenchmark.__proto__ || Object.getPrototypeOf(KerasJSBenchmark)).call(this, name, numIteration));
...
var WebDNNBenchmark = function(_BaseBenchmark2) {
    ...
    function WebDNNBenchmark(name, numIteration, backend, flagOptimized) {
        _classCallCheck(this, WebDNNBenchmark);

        var _this4 = _possibleConstructorReturn(this, (WebDNNBenchmark.__proto__ || Object.getPrototypeOf(WebDNNBenchmark)).call(this, name, numIteration));
...
function run() {
    ...
    var webdnnOptimizedGPU = new WebDNNBenchmark('WebDNN(WebGPU) + Optimize', N, 'webgpu', true);
    var webdnnNonOptimizedGPU = new WebDNNBenchmark('WebDNN(WebGPU)', N, 'webgpu', false);
    var webdnnOptimizedCPU = new WebDNNBenchmark('WebDNN(WebAssembly) + Optimize', N, 'webassembly', true);
    var webdnnNonOptimizedCPU = new WebDNNBenchmark('WebDNN(WebAssembly)', N, 'webassembly', false);
    ...
    Promise.resolve().then(function() {
        return webdnnOptimizedGPU.runAsync().then(summaryHandler);
    }).then(function() {
        return webdnnNonOptimizedGPU.runAsync().then(summaryHandler);
    }).then(function() {
        return webdnnOptimizedCPU.runAsync().then(summaryHandler);
    }).then(function() {
        return webdnnNonOptimizedCPU.runAsync().then(summaryHandler);
    }).then(function() {
        return kerasGPU.runAsync().then(summaryHandler);
    }).then(function() {
        return kerasCPU.runAsync().then(summaryHandler);
    }).then(function() {
