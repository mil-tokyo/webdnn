...
def train_model(save_path, sample_output_path):
    ...
    for i in range(export_size):
        test_samples_json.append({"x": x_export[i].tolist(), "y": int(y_export[i]), "orig_pred": float(orig_pred_export[i, 0])})
...
def main():
    ...
    convert_keras_command = f"python ../../bin/convert_keras.py {original_model_path} --input_shape '(1,{maxlen})' --out {args.out}"
    if args.encoding:
