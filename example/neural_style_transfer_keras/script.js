...
async function initialize() {
    try {
        ...
        dnn = await WebDNN.load("./output", {backendOrder:['webassembly']});
...
function getImageData() {
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            inputView[(y * w + x) * 3 + 2] = pixelData[(y * w + x) * 4] - 123.68;//R
            inputView[(y * w + x) * 3 + 1] = pixelData[(y * w + x) * 4 + 1] - 116.779;//G
            inputView[(y * w + x) * 3 + 0] = pixelData[(y * w + x) * 4 + 2] - 103.939;//B
        }
    }
}
...
function setImageData() {
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            imageData.data[(y * w + x) * 4] = outputView[(y * w + x) * 3 + 2] + 123.68;//R
            imageData.data[(y * w + x) * 4 + 1] = outputView[(y * w + x) * 3 + 1] + 116.779;//G
            imageData.data[(y * w + x) * 4 + 2] = outputView[(y * w + x) * 3 + 0] + 103.939;//B
